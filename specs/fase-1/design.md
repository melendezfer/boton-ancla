...
| HM-10 | ✅ Guía de desplazamiento en dos variantes (prop `guiaDesplazar` del proveedor, `"ancla"` por defecto). Núcleo puro: `indicadorDesplazamiento(dy, params)` → `{ direccion: -1 \| 0 \| 1, llenado: 0…1 }` (llenado = velocidad / máxima) y `posicionGuiaArriba(...)` → centro x y borde superior de la cápsula (corrida `GUIA_CORRIMIENTO` hacia el centro, fondo a `GUIA_SEPARACION` sobre `origen.y − R_MAX_DESPLAZAR` y sobre el ancla, sin salir por arriba). El adaptador no redibuja React: el bucle escribe `data-direccion` y `--ba-llenado` en el ancla (B) o mueve el punto de la cápsula (A). El anillo es un SVG del tamaño de `D_ACTIVO` (sobre el borde del ancla, no por fuera) con `stroke-dashoffset`. La columna del ancla mide `MARGEN_LATERAL + D_ACTIVO` desde el borde; ni el anillo ni la cápsula salen de ella (probado en el núcleo y en E2E). Demo: selector en Ajustes. | spec RF-18, §6, §7, §12 |
| HM-11 | ✅ Joystick libre en el mapa (experimental). La máquina no cambia: la entrada sigue siendo la fila 41/42 y `desplazando` ya guarda `origen` y `ultimo` en 2D. `Geometry.modoDesplazar: "vertical" \| "libre"` (lo pone el adaptador según el objetivo) y la métrica `scroll_start {modo}`. Núcleo puro: `velocidadJoystick(dx, dy, params, reducido)` → `{ vx, vy }` (misma curva que `velocidadDesplazamiento`, con la distancia) e `indicadorJoystick(dx, dy, params)` → `{ angulo \| null, llenado }`. Adaptador: `useAnchorPan(mover)` registra el objetivo libre en el mismo lugar que `useAnchorScroll` (uno a la vez; con capa, manda la capa); prop `desplazarLibre`; el bucle acumula fracciones en x e y y llama `mover(dx, dy)`; si devuelve `false`, vibra una vez (borde). Guía: variante "ancla" = una flecha girada con `--ba-giro`; variante "arriba" = círculo de `R_MAX_DESPLAZAR + 24` px (el punto recorre la mitad de la distancia) ubicado con `posicionGuiaArriba`. En el mapa el círculo sí sale de la columna del ancla (no hay texto de lista que tapar). Demo: interruptor "Mover el mapa con el ancla" y el mapa falso registra `mover`. | spec RF-19, HU-13, §7, §9, §12 |
| HM-12 | ✅ **Decidido; 12a en curso, 12b después de probar 12a.** HM-12a (mapa + zoom B): máquina con `desplazando.apuntado` (evento `APUNTAR`), estado transitorio `elegido`, estado `ajustando` (fila 45–50), `abierto_gesto.tPresel` y su plazo; núcleo puro `resolverApuntado` (imán, grupos) y `zoomParaSeparar`; adaptador con mira, ancla encendida, freno, imán y zoom automático en el bucle; `onSlide`/`slideHint` en `AnchorAction`. Propuesta original: *Máquina:* `desplazando` gana `submodo: "desplazar" \| "apuntar"` y `apuntado: string \| null`. El adaptador calcula qué hay en el objetivo (hit-test de la mira o de la franja) y lo envía con un evento nuevo `APUNTAR {id}` (como hoy la geometría llega con los eventos: el núcleo sigue sin DOM). En listas, `POINTER_MOVE` con `\|dx\|` hacia el centro > `UMBRAL_APUNTAR` pasa a `apuntar`, y vuelve a `desplazar` bajo `UMBRAL_APUNTAR − HISTERESIS_APUNTAR` (sin parpadeo). En `apuntar`, cada `PASO_APUNTAR` px de pulgar = un elemento. `POINTER_UP` con `apuntado` **y el pulgar en la zona muerta** (ya frenado) → `ejecutando` una selección que abre la capa (HM-08); si se suelta en movimiento, solo se detiene (evita elegir sin querer al soltar mientras se viaja). *Choques:* HM-03/HM-08: la selección es una capa más de la pila; "Cerrar" a 90° la cierra y deselecciona; con esa capa abierta no hay joystick del mapa (RF-19). HM-09: el movimiento lateral, hoy ignorado, pasa a significar "apuntar"; el umbral y la histéresis evitan que la deriva natural del pulgar cambie de modo. HM-11: en el mapa todo movimiento es "mover", así que la mira está siempre activa; elegir exige frenar. Modo experto (D-08/C-05): no cambia, porque la entrada sigue siendo hacia abajo y el experto va al arco. Descanso: se entra al joystick desde el descanso igual que hoy. *Pines juntos:* (1) freno `FRENO_APUNTAR` dentro de `IMAN_RADIO` y radio del imán = mín(`IMAN_RADIO`, mitad de la distancia al pin vecino), nunca bajo `IMAN_RADIO_MIN`; (2) pines a menos de `GRUPO_DISTANCIA` px en pantalla forman un grupo: se apunta al grupo y soltar abre una capa con su lista; (3) con la mira quieta `T_ZOOM_GRUPO` sobre un grupo, zoom automático hasta separarlos. *Cómo se ve:* reposo = ícono de la sección; desplazar = flecha + anillo (HM-10); mira activa sin objetivo = mira tenue y ancla con flecha; con objetivo = ancla encendida (acento) con el ícono y el nombre en la banda (HM-02); grupo = ícono de grupo + "N negocios"; seleccionado = capa abierta con su nombre en el centro (HM-08). | spec §12 (HM-12) |
| HM-14 | ✅ Regla de 90°: `AnchorAction.atTop` → `OrderedAction.arriba` (solo si la pantalla no tiene "Atrás"); `assignActions` la fija al extremo "arriba" igual que "Atrás"/"Cerrar" (con una capa, `pantallaDeCapa` pone "Atrás"→"Cerrar" y `atTop` se ignora). `validateScreen`: a lo sumo una `atTop`; exactamente una si no hay "Atrás" y hay 2+ acciones; debe ser normal y sin `onSlide`. | spec RF-22, D-10 |
| HM-12b | ⏳ **En curso.** *Máquina:* `desplazando` gana `submodo: "desplazar" \| "apuntar"` y `origenApuntar`; `Geometry.apuntarLista` (lo pone el adaptador si la lista ofrece elementos y el interruptor está activado). MOVE en modo vertical: hacia el centro (según la mano) > `UMBRAL_APUNTAR` desde `origen.x` → `apuntar` (`origenApuntar` = punto); < `UMBRAL_APUNTAR − HISTERESIS_APUNTAR` → `desplazar` (y `apuntado` = null). UP en `apuntar` con `apuntado` → `elegido`. Pura: `pasosApuntar(dy, params)` = elementos a avanzar. *Adaptador:* `apuntar` en `useAnchorScroll` y en la capa; el bucle, en `apuntar`, no desplaza por velocidad: calcula el índice (`base + pasos`), marca `data-ba-foco` en el elemento, envía `APUNTAR`, y desliza la lista para centrarlo en la franja (centro de la parte visible del contenedor); si la lista ya no puede desplazarse (un extremo), la franja se dibuja sobre el elemento. *Demo:* hojas apiladas (la de abajo sigue montada y oculta con `visibility`, así conserva su `scrollTop`); Favoritos, Ofertas y el grupo ofrecen sus filas; la carta (ventana) ofrece sus platos. | spec RF-23 |
...
| 34 | ejecutando / cancelado / bloqueado_sensible / elegido | COMPLETADO | — | reposo | el adaptador ya hizo el efecto |
...
| 45 | desplazando | APUNTAR {apuntado} | — | desplazando (actualiza `apuntado`) | RF-21 |
| 46 | desplazando | POINTER_UP | `apuntado`, modo libre y pulgar en `R_MUERTA_DESPLAZAR` desde `origen` | elegido {apuntado} (transitorio → capa) | RF-21 |
| 47 | abierto_gesto | TICK (o MOVE tardío) | `presel` es deslizador y `t − tPresel ≥ T_ESPERA_DESLIZADOR` | ajustando {id, origen = último punto} | RF-20 |
